...
def soap_client():
    ...
    try:
        ...
        pytest.skip(f"Could not connect to SOAP service at {WSDL_URL}. Make sure docker-compose is running. Error: {e}")
...
def test_soap_credit_score_range(soap_client):
    ...
    for client_id in test_clients:
        ...
        assert 0 <= response.credit_score <= 1000, f"Score {response.credit_score} out of range for {client_id}"
...
def test_soap_financial_calculations_consistency(soap_client):
    ...
    assert status == expected_status, f"Decision inconsistent: score={score}, income={income}, expenses={expenses}"
...
def test_soap_service_operations(soap_client):
    ...
    assert hasattr(service, 'VerifySolvency'), "VerifySolvency operation not found in WSDL"