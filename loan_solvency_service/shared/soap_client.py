...
class InternalSoapClient:
    ...
    def _initialize_client(self):
        ...
        for attempt in range(self.max_retries):
            try:
                logger.info(f"Connecting to {self.service_name} at {self.wsdl_url} (attempt {attempt + 1}/{self.max_retries})")
                ...
                if attempt < self.max_retries - 1:
                    ...
                    logger.error(f"Could not connect to {self.service_name} after {self.max_retries} attempts")
                    raise
    ...
    def call_operation(self, operation_name, correlation_id=None, **kwargs):
        ...
        try:
            logger.info(f"[{correlation_id}] Calling {self.service_name}.{operation_name} with params: {kwargs}")
            ...
            logger.info(f"[{correlation_id}] {self.service_name}.{operation_name} completed in {latency:.2f}ms")
            
            return result, latency
            
        except Fault as e:
            latency = (time.time() - start_time) * 1000
            logger.error(f"[{correlation_id}] {self.service_name}.{operation_name} failed with SOAP Fault: {e}")
            raise
        except Exception as e:
            latency = (time.time() - start_time) * 1000
            logger.error(f"[{correlation_id}] {self.service_name}.{operation_name} failed: {e}")
            raise