...
class ExplanationService(SoaServiceBase):
    ...
    def Explain(credit_score, monthly_income, monthly_expenses, debt, late_payments, has_bankruptcy):
        """
        ...
        if credit_score >= 800:
            score_explanation = f"Excellent credit score of {credit_score}. Strong creditworthiness."
        elif credit_score >= 700:
            score_explanation = f"Good credit score of {credit_score}. Acceptable credit risk."
        elif credit_score >= 500:
            score_explanation = f"Fair credit score of {credit_score}. Moderate credit risk."
        else:
            score_explanation = f"Poor credit score of {credit_score}. High credit risk."
        
        # 2. Income vs Expenses Explanation
        net_income = monthly_income - monthly_expenses
        if net_income > 1000:
            income_explanation = f"Strong financial position with ${net_income:.2f} monthly surplus."
        elif net_income > 0:
            income_explanation = f"Tight budget with only ${net_income:.2f} monthly surplus."
        elif net_income == 0:
            income_explanation = "Break-even situation. Income exactly matches expenses."
        else:
