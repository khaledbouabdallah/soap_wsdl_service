...
from loan_solvency_service.shared.datamodels import ClientId, ClientIdentity, map_client_to_models
...
class ClientDirectoryService(SoaServiceBase):
    ...
    @srpc(ClientId, _returns=ClientIdentity)
    def GetClientIdentity(client_id):
        ...
        try:
            ...
            raise ClientNotFoundFault(f"Client with ID '{client_id}' not found in directory.")
        finally:
            db.close()